...
export type Subscribe<T extends State> = {
  (listener: StateListener<T>, getter?: any): () => void;
};
...
const createStoreImpl: CreateStoreImpl = (createState) => {
  ...
  const setState: SetState<TState> = (partial, replace) => {
    ...
    if (nextState !== state) {
      ...
      listeners.forEach((listener) => {
        if (
          listener.getter &&
          listener.getter(state) !== listener.getter(previousState)
        ) {
  ...
  const subscribe: Subscribe<TState> = (
    callback: StateListener<TState>,
    getter: StateGetter<TState>
  ) => {
    const listener = { callback, getter };
